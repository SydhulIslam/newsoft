		...
		<div class="row">
			<div class="col-8 mx-auto text-center">
				...
				<ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
					<li class="list-inline-item"><a href="{{'home'}}">Home</a>
					</li>
		...
		<div class="row">
			<div class="col-lg-9">
				<div class="me-lg-4">
					<div class="row gy-5">
							...
							<div class="col-md-6" data-aos="fade">
								<article class="blog-post">
									...
									<div class="pt-4">
										...
										<h2 class="h4"><a class="text-black" href="{{ route('blog-details', $blog->slug )}}">{{ $blog -> id}}. {{ $blog->title}}</a></h2>
										<p>{{$blog->excerpt}}</p>
										<a href="{{ route('blog-details', $blog->slug )}}" class="text-primary fw-bold" aria-label="Read the full article by clicking here">Read More</a>
									</div>
								</article>
							</div>
						...
						@else
							<h2 class="post-title text-center" >No Possts Found. Check After</h2>
						@endif
			...
			<div class="col-lg-3">
				...
				<div class=" col-md-12 newsletter-form">
					<h5 class="mb-4 text-primary font-secondary">Search</h5>
					<div class="pe-0 pe-xl-0">
						<form action="{{route('blog.index')}}" method="get" name="" >
							<div class="input-group mb-5">
								<input type="text" name="search" class="form-control shadow-none bg-white border-end-0" placeholder="Search Bar">
								<span class="input-group-text border-0 p-0">
									<button class="input-group-text border-0 bg-primary" type="submit" name="" >
				...
				<div class="widget widget-categories">
					...
					<ul class="list-unstyled widget-list">
						@foreach ($categorys as $category)
							<li><a href="#!">{{ $category -> name}} <small class="ml-auto">({{ $category -> id}})</small></a>
							</li>
						@endforeach
						...
						<li class="d-flex widget-post align-items-center">
							<a class="text-black" href="/blog/elements/">
								<div class="widget-post-image flex-shrink-0 me-3">
									<img class="rounded" loading="lazy" decoding="async" src="{{$blog->thumbnail}}" alt="Post Thumbnail">
								</div>
							</a>
							<div class="flex-grow-1">
								<h5 class="h6 mb-0"><a class="text-black" href="{{ route('blog-details', $blog->slug )}}">E{{ $blog -> id}}. {{ $blog->title}}</a></h5>
								<small>{{ date('d F', strtotime($blog->created_at)) }}</small>
							</div>
						</li>
					</ul>
					@endforeach
